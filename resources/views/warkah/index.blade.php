    ...
    <div class="mt-6 bg-white shadow-md rounded-lg p-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <form action="{{ route('warkah.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <label
                    class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 hover:scale-105 cursor-pointer group">
...
@section('content')
    <div class="space-y-6">
        ...
        <form method="GET" action="{{ route('warkah.index') }}"
            class="bg-gradient-to-r from-white via-gray-50 to-white rounded-xl shadow-lg border border-gray-200 p-4 sm:p-6 transform transition-all duration-300 hover:shadow-xl">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="kurun_waktu_berkas"
                        class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:ring-4 focus:ring-blue-200 focus:border-blue-500 transition">
                        <option value="">Semua Tahun</option>
                        @foreach ($tahunList as $tahun)
                            <option value="{{ $tahun }}"
                                {{ request('kurun_waktu_berkas') == $tahun ? 'selected' : '' }}>
                                {{ $tahun }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="ruang_penyimpanan_rak"
                        class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:ring-4 focus:ring-blue-200 focus:border-blue-500 transition">
                        <option value="">Semua Lokasi</option>
                        @foreach ($lokasiList as $rak)
                            <option value="{{ $rak }}"
                                {{ request('ruang_penyimpanan_rak') == $rak ? 'selected' : '' }}>
                                {{ $rak }}
                            </option>
                        @endforeach
                    </select>
                        ...
                        @forelse ($warkah as $index => $item)
                            <tr
                                ...
                                <td class="px-6 py-4 text-sm text-gray-600 truncate max-w-xs"
                                    title="{{ $item->uraian_informasi_arsip }}">
                                    ...
                                    <div class="flex justify-center gap-2">
                                        ...
                                        @if ($item->status == 'Tersedia')
                                            <button type="button"
                                                ...
                                                data-url="{{ route('warkah.destroy', $item->id) }}" title="Hapus">
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if ($warkah->onFirstPage())
                                <span
                                    class="w-12 h-12 flex items-center justify-center rounded-xl bg-gray-100 text-gray-400 cursor-not-allowed">
                            ...
                            @for ($i = 1; $i <= $lastPage; $i++)
                                @if ($i == 1 || $i == $lastPage || ($i >= $currentPage - 1 && $i <= $currentPage + 1))
                                    @if ($i == $currentPage)
                                        <span
                                            class="w-12 h-12 flex items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 text-white font-bold shadow-lg">
                                            {{ $i }}
                                        </span>
                                    @else
                                        <a href="{{ $warkah->url($i) }}"
                        ...
                        <!-- Go to Page Input (Kanan) -->
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-medium text-gray-700">Ke Halaman:</span>
                            <form method="GET" action="{{ route('warkah.index') }}" class="flex items-center gap-2">
                                @foreach (request()->except('page') as $key => $value)
                                    <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                @endforeach

                                <input type="number" name="page" min="1" max="{{ $warkah->lastPage() }}"
                                    value="{{ $warkah->currentPage() }}" placeholder="Page"
                                    class="w-20 px-3 py-2 bg-white border-2 border-gray-300 rounded-xl focus:ring-4 focus:ring-blue-200 focus:border-blue-500 text-center font-semibold text-gray-700 transition-all duration-200"
                                    onkeypress="if(event.key === 'Enter') { this.form.submit(); }">
                                <button type="submit"
                                    class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all duration-200">
        ...
        <div class="md:hidden space-y-4">
            @forelse ($warkah as $item)
                <div
                    ...
                    <p class="text-sm text-gray-700 mb-3"><strong>Uraian:</strong>
                        {{ Str::limit($item->uraian_informasi_arsip, 80) ?: '-' }}</p>
